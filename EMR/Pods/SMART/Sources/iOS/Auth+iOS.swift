...
extension Auth {
	...
	*/
	func authorizeWith(oauth: OAuth2, properties: SMARTAuthProperties) {
        ...
        authContext = UIApplication.sharedApplication().keyWindow?.rootViewController?.presentedViewController != nil ? UIApplication.sharedApplication().keyWindow?.rootViewController?.presentedViewController : authContext;
